...
From my experience, it was easiest to use ArcGIS Pro to edit/manipulate data and layers, and MapViewer (via the ArcGIS Online Portal) to consolidate and edit the Map itself.
...
6.  When prompted, host it as a feature layer (both file and feature layer, not just file).
    ...
    It is feature data that is stored and managed by the hosting server. So if you downloaded data from the Bureau of Indian Affairs, for example, you'd want to then import the data files onto the map, and then convert them to
    :::

### Adding a Layer to Map (via ArcGIS Pro)

1.  In the Tribal Water Data Initiatives Map
