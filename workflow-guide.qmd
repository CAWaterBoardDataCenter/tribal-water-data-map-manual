...
1.  Locate the relevant data download (ideally in GeoJSON, Shapefile, or CSV format), or URL to the data source.
    ...
    - For CSV files, see the [Converting a CSV Table to Point Features] section so that ArcGIS Pro can pull the spatial data into the map.
...
#### Converting a CSV Table to Point Features

A CSV containing latitude and longitude coordinates can be converted into a spatial point feature class using the XY Table to Point tool in ArcGIS Pro.

1.  Add the CSV to ArcGIS Pro using the instructions provided in [Adding a Layer to Map (via ArcGIS Pro)].

2.  Navigate to the Contents Pane on the left side of your screen. The CSV can be found under the *Standalone Tables* section of the Contents Pane.

3.  Right-click the CSV file.

    - Hover over *Create Points From Table*

    - Select XY Table to Point

      ![](images/CSV-Points.png){width="450"}

4.  In the XY Table to Point pop-up:

    - Use the CSV you want to convert to points as the Input Table

    - Rename the Output Feature Class

    - Ensure the X Field is the table's longitude field and the Y Field is the latitude field. Leave Z Field blank.

    - Accept the default Coordinate System.

    - Click OK.

      ![](images/CSV-Points2.png){width="350"}
